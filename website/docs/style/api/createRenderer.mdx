---
title: createRenderer
order: 1
---

Creates the CSS function required to make it work.

## The Gist

```ts
import {
  createRenderer,
  prefixerPlugin,
  fallbackValuePlugin,
  prefixerFallbacks,
} from '@weser/style'

const css = createRenderer({
  plugins: [fallbackValuePlugin(prefixerFallbacks), prefixerPlugin()],
})

// vendor prefixed and browser-compatible position: sticky
const style = css({
  display: 'flex',
  flexDirection: 'column',
  color: 'blue',
  position: 'sticky',
  appearance: 'none',
  ':hover': {
    color: 'red',
  },
  '@media (min-width: 800px)': {
    flexDirection: 'row',
  },
})
```

When using [precompileConditions](/style/api/precompileConditions) to share condition styles across renderers:

```ts
import { createRenderer, precompileConditions } from '@weser/style'

const [conditionsNode, precompiledConditions] = precompileConditions([
  ':hover',
  '@media (min-width: 800px)',
])

const css = createRenderer({
  plugins: [/* ... */],
  precompiledConditions,
})
```

## Parameters

| Parameter             | Type                                                                                                   | Description                                                                 |
| --------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| plugins               | `Array<Plugin>`                                                                                        | An array of plugins to apply to the style.                                  |
| mergeStyle            | [assignStyle](https://github.com/robinweser/css-in-js-utils?tab=readme-ov-file#assignstylebase-extend) | A function that merges the style object.                                    |
| precompiledConditions | `Record<string, string>`                                                                               | Pre-compiled condition flags from [precompileConditions](/style/api/precompileConditions). Use to share condition styles across renderers. |

### mergeStyle

An opinionated style merging function similar to `Object.assign` that handles style objects with special logic for nested styles and CSS properties. It's used internally to merge multiple style objects passed to the CSS function and is also available to plugins via the context.

## Returns

(`CSSFunction`) A function that takes one or more style objects or nested arrays of style objects and returns a single resolved style object.
